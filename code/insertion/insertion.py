...
password = "password"
dbname = "neo4j"
jar_path = "/Users/enriquegomeztagle/jars/neo4j-connector-apache-spark_2.12-5.3.8_for_spark_3.jar"
me = "Enrique Gómez Tagle"
...
# %%
def write_nodes(df, label, key_cols, mode="Append"):
    (
...
def clean_name(colname: str):
    # 1) Quitar sufijos académicos tras coma
    no_titles = regexp_replace(col(colname), r",\s*(M?Sc\.?|B?Sc\.?|PhD\.?|MBA\.?)", "")
    # 2) Eliminar todo lo que esté entre paréntesis
    no_parens = regexp_replace(no_titles, r"\(.*?\)", " ")
    # 3) Quitar puntos sobrantes pegándolos al fragmento previo
    ...
    # 5) Normalizar mayúsculas/minúsculas
...
positions_df = (
    ...
    .csv("KA-DataExport/Positions.csv")
    ...
    .withColumn("user", F.lit("Enrique Gómez Tagle"))
    .select(
        "user",
        "company",
...
education_df = (
    spark.read.option("header", True)
    .csv("KA-DataExport/Education.csv")
...
langs_df = (
    spark.read.option("header", True)
    .csv("KA-DataExport/Languages.csv")
...
user_langs_df = langs_df.withColumn("user", F.lit("Enrique Gómez Tagle")).select(
...
raw_certs_df = (
    spark.read.option("header", True)
    .csv("KA-DataExport/Certifications.csv")
    .withColumnRenamed("Name", "name")
...
user_certs_df = (
    raw_certs_df.withColumn("user", F.lit("Enrique Gómez Tagle"))
...
raw_jobs_df = (
    spark.read.option("header", True)
    .csv("KA-DataExport/Saved Jobs.csv")
...
user_saved_df = raw_jobs_df.select(
    F.lit("Enrique Gómez Tagle").alias("user"), "id", "savedDate"
).dropDuplicates(["user", "id", "savedDate"])

# %%
skills_df = (
    spark.read.option("header", True)
    .csv("KA-DataExport/Skills.csv")
...
follows_df = (
    spark.read.option("header", True)
    .csv("KA-DataExport/Company Follows.csv")
...
# %%
raw = spark.read.option("header", True).csv("KA-DataExport/Connections.csv")
...
inv_df = (
    spark.read.option("header", True)
    .csv("KA-DataExport/Invitations.csv")
...
conns_df = (
    ...
    .withColumn("nameB", F.lit("Enrique Gómez Tagle"))
...
# ——————————————————————————————————————————————————————————————
# 1) Relaciones CONNECTED que faltan en Neo4j
neo_conns = (
...
conns_df = (
    spark.read.option("header", True)
    .csv("KA-DataExport/Connections.csv")
    .withColumn("nameA", concat_ws(" ", col("First Name"), col("Last Name")))
    .withColumn("nameB", lit("Enrique Gómez Tagle"))
...
faltantes_conns = conns_df.join(
    neo_conns, on=["nameA", "nameB", "since"], how="left_anti"
)
print(f"RELACIONES CONNECTED faltantes en Neo4j: {faltantes_conns.count()}")
faltantes_conns.show(truncate=False)


# ——————————————————————————————————————————————————————————————
# 2) Usuarios que faltan en Neo4j
# Primero, extraemos todos los usuarios de tu CSV de conexiones:
users_df = (
    spark.read.option("header", True)
    .csv("KA-DataExport/Connections.csv")
...
# Leemos los nodos :User que ya existen en Neo4j
neo_users = (
    spark.read.format("org.neo4j.spark.DataSource")
    .option("labels", ":User")
...
missing_users = users_df.join(neo_users, on="name", how="left_anti")
print(f"USUARIOS faltantes en Neo4j: {missing_users.count()}")
missing_users.show(truncate=False)


# ——————————————————————————————————————————————————————————————
# 3) Compañías que faltan en Neo4j
# Asume que ya tienes 'companies_df' como el catálogo de companies (p. ej. union de posiciones, follows, jobs, apps…)
# Sino, reacumúlalo igual que antes:
# companies_df = positions_df.select("company").union(...).withColumnRenamed("company","name")…

# Leemos los nodos :Company de Neo4j
neo_companies = (
    spark.read.format("org.neo4j.spark.DataSource")
...
print(f"COMPAÑÍAS faltantes en Neo4j: {missing_companies.count()}")
